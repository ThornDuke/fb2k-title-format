...
[
  ...
  },
  {
    ...
    "example": [""],
    "ref-link": "hhttps://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$greater(a,b)"
  },
  {
    "token": "max",
    "realm": "FB2K",
    ...
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$max(a,b)"
  },
  {
    "token": "min",
    "realm": "FB2K",
    ...
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$min(a,b)"
  },
  {
    "token": "mod",
    "realm": "FB2K",
    ...
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$mod(a,b)"
  },
  {
    "token": "mul",
    "realm": "FB2K",
    ...
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$mul(a,b)"
  },
  {
    "token": "muldiv",
    "realm": "FB2K",
    ...
    "snippet-body": [""],
    "description": [
      "Multiplies `a` and `b`, then divides by `c`. The result is rounded to the
  nearest integer."
    ],
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$muldiv(a,b,c)"
  },
  {
    "token": "rand",
    "realm": "FB2K",
    ...
    "snippet-body": [""],
    "description": [
      "Generates a random number in the range from 0 to 2<sup>32</sup>-1. Available only in sort-related contexts, such as the `.` menu command."
    ],
    "example": [""],
    "ref-link": "https://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Title_Formatting_Reference#$rand()"
  },
  {
    "token": "sub",
    "realm": "FB2K",
    ...
    "sign": "$sub(a,b)",
    "snippet-body": [""],
    "description": ["Subtracts `b` from `a`.", "Can be used with an arbitrary number of arguments. `$sub(a,b,...)` is the same as `$sub($sub(a,b),...)`."],
    "example": [""],
...
<p>
  Can be used with an arbitrary number of arguments. `)` is the same as `)`.
</p>
<h3>
  <span id=".24and.28expr.2C_....29"></span
  ><span class="mw-headline" id="$and(expr,_...)">$and(expr, ...)</span>
</h3>
<p>
  Logical And of an arbitrary number of arguments. Returns `e`, if and only if
  all `r` arguments evaluate to `e`.
</p>
<h3>
  <span id=".24or.28expr.2C_....29"></span
  ><span class="mw-headline" id="$or(expr,_...)">$or(expr, ...)</span>
</h3>
<p>
  Logical Or of an arbitrary number of arguments. Returns `e`, if at least one
  expression evaluates to `e`.
</p>
<h3>
  <span id=".24not.28expr.29"></span
  ><span class="mw-headline" id="$not(expr)">$not(expr)</span>
</h3>
<p>
  Logical Not. Returns the logical opposite of EXPR: `e`, if `r` is `e` and `e`
  if `r` is false.
</p>
<h3>
  <span id=".24xor.28expr.2C....29"></span
  ><span class="mw-headline" id="$xor(expr,...)">$xor(expr,...)</span>
</h3>
<p>
  Logical Exclusive-or of an arbitrary number of arguments. Returns `e`, if an
  odd number of arguments evaluate to `e`.
</p>
<p>
  Special case: `)` returns `e`, if EXPR1 or EXPR2 is `e`. If both expressions
  are true, returns `e`.
</p>
<h3>
  <span id=".24if.28cond.2Cthen.29"></span
  ><span class="mw-headline" id="$if(cond,then)">$if(cond,then)</span>
</h3>
<p>
  If `d` evaluates to `e`, the `n` part is evaluated and its value returned.
  Otherwise, `e` is returned.
</p>
<p>
  Plain strings are FALSE. Field lookups and functions can introduce a boolean
  value of TRUE. <br />
  <u>Examples</u>:<br />
  <u>1</u>
  <code> </code>
...
<p>
  <code></code><br />
  The last one would display the value of %rating% plus one, if and only if
  %rating% is set for the track.
</p>
<p>
  <u>2</u>
...
<p><code></code></p>
<h3>
  <span id=".24if.28cond.2Cthen.2Celse.29"></span
  ><span class="mw-headline" id="$if(cond,then,else)">$if(cond,then,else)</span>
</h3>
<p>
  If `d` evaluates to `e`, the `n` part is evaluated and its value returned.
  Otherwise, the `e` part is evaluated and its value returned.
</p>
<h3>
  <span id=".24if2.28expr.2Celse.29"></span
  ><span class="mw-headline" id="$if2(expr,else)">$if2(expr,else)</span>
</h3>
<p>
  Like `)` except that `r` is only evaluated once. In other words, if expression
  `r` is true, `r` is returned, otherwise the `e` part is evaluated and `r` is
  returned as true.
</p>
<h3>
  <span id=".24if3.28a1.2Ca2.2C....2CaN.2Celse.29"></span
  ><span class="mw-headline" id="$if3(a1,a2,...,aN,else)"
    >$if3(a1,a2,...,aN,else)</span
  >
</h3>
<p>
  Evaluates arguments `1` ... `N`, until one is found that evaluates to `e`. If
  that happens, its value is returned. Otherwise the `e` part is evaluated and
  its value returned.
</p>
<h3>
  <span id=".24ifequal.28int1.2Cint2.2Cthen.2Celse.29"></span
  ><span class="mw-headline" id="$ifequal(int1,int2,then,else)"
    >$ifequal(int1,int2,then,else)</span
  >
</h3>
<p>
  Compares the integer numbers `1` and `2`, if `1` is equal to `2`, the `n` part
  is evaluated and its value returned. Otherwise the `e` part is evaluated and
  its value returned.
</p>
<h3>
  <span id=".24ifgreater.28int1.2Cint2.2Cthen.2Celse.29"></span
  ><span class="mw-headline" id="$ifgreater(int1,int2,then,else)"
    >$ifgreater(int1,int2,then,else)</span
  >
</h3>
<p>
  Compares the integer numbers `1` and `2`, if `1` is greater than `2`, the `n`
  part is evaluated and its value returned. Otherwise the `e` part is evaluated
  and its value returned.
</p>
<h3>
  <span id=".24iflonger.28str.2Cn.2Cthen.2Celse.29"></span
  ><span class="mw-headline" id="$iflonger(str,n,then,else)"
    >$iflonger(str,n,then,else)</span
  >
</h3>
<p>
  Compares the length of the string `r` to the number `n`, if `r` is longer than
  `n` characters, the `n` part is evaluated and its value returned. Otherwise
  the `e` part is evaluated and its value returned.
</p>
<h3>
  <span id=".24select.28n.2Ca1.2C....2CaN.29"></span
  ><span class="mw-headline" id="$select(n,a1,...,aN)"
    >$select(n,a1,...,aN)</span
  >
</h3>
<p>
  If the value of `n` is between 1 and N, `n` is evaluated and its value
  returned. Otherwise `e` is returned.
</p>
<h2>
  <span class="mw-headline" id="String_functions">String functions</span>
</h2>
<p>
